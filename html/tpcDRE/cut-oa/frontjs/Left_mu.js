$(document).ready(function() {
	...
	// 主選單按鈕開合
$('.nav-close').data('status', 'open').bind('click', function(e) {
		e.preventDefault();
	nav_reset();
...
//	}
	...
	// 主選單第二層開合
	var num;
	$('#navmu h2 > a').bind('click', function(e) {
		e.preventDefault();
		...
		if($eleLi.find('.nav-second')[0]) {
		    ...
		    $eleLi.data('status', 'close').addClass('open').find('.nav-second').show();
		   	$pageWrap.css('min-height', $('.Left_mu').height() + 48);
		   	num = $pageWrap.height();
		  } else {
	...
	$('#navmu .reset li .nav-second .reset li > a').bind('click', function(e) {
		e.preventDefault();
	...
	$('.search-button').bind('click', function(e) {
		...
		$('.main-panel').removeClass('toggle');
		$('.nav-close').data('status', 'open').find('.fa-times').removeClass('fa-times').addClass('fa-bars');
		$navPanel.removeClass('toggle');
		$pageWrap.css('min-height', 'auto');
	});

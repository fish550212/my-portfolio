$(document).ready(function() {
	var $win = $(window),
			$pageWrap = $('.page-wrap'),
			$navPanel = $('.nav-panel01'),
			$nav = $('#nav'),
			$navLi = $nav.find('li').data('status', 'open'),
			$searchPanel = $('.search-panel01');

	function nav_reset() {
		$navLi.data('status', 'open').removeClass('open').find('.nav-second01').hide();
	}
	...
	// 主選單按鈕開合
	$('.nav-button01').data('status', 'open').bind('click', function(e) {
		e.preventDefault();
		nav_reset();
	...
	$('#nav h2 > a').bind('click', function(e) {
		...
		if($eleLi.find('.nav-second01')[0]) {
			if ($eleLi.data('status') == 'open') {
		    $eleLi.data('status', 'close').addClass('open').find('.nav-second01').show();
		   	$pageWrap.css('min-height', $('.nav-panel01').height() + 48);
		   	num = $pageWrap.height();
		  } else {
		    $eleLi.data('status', 'open').removeClass('open').find('.nav-second01').hide();
		    var $secondH = $eleLi.find('.nav-second01').height() + 10;

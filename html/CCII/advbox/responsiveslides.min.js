    ...
    return this.each(function () {
      ...
      if (1 < e.size()) {
        ...
        if (a.nav) {
          ...
          // 原本是加在外面，改成加在每個 li 裡面
          e.each(function () {
            c(this).append(g);
          });
          var $nav = f.find(".prev, .next");

          $nav.on("click", function (evt) {
            evt.preventDefault();
            if (c("." + j).queue("fx").length) return;

            var isPrev = c(this).hasClass("prev");
            var target = isPrev ? (n - 1 + C) % C : (n + 1) % C;

            if (a.pager || a.manualControls) r(target);
            z(target);
            if (!a.pauseControls) m();
          });
